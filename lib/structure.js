...
var number = node('number',
    ...
    clojure_string = node('string',
        ['open' , literal('"')                        ],
        ['body' , many0(alt(not1(oneOf('\\"')), // TODO change to `chars` to avoid confusion with `hasBody`
                            seq(literal('\\'),
                                cut('escape', item))))],
        ['close', cut('"', literal('"'))              ]),
    ...
    regex = node('regex',
        ['open' , string('#"')                        ],
        ['body' , many0(alt(not1(oneOf('\\"')), // TODO change to `chars`
                            seq(literal('\\'),
                                cut('escape', item))))],
        ['close', cut('"', literal('"'))              ]);
...
var clojure = node('clojure',
    ['junk', junk       ],
    ['body', many0(form)]); // TODO change to `forms` to differentiate from `hasBody`
