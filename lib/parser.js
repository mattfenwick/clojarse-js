...
// why is this not implemented as a separate file?
//   because it is tightly coupled to the parse tree create by the parser
...
function dirtyParse(input) {
    ...
    return clojure.parse(input, init_position);
}
...
function parse(input) {
    ...
    if ( parsed.status === 'success' ) {
        return parsed.value.result.map(cleanParseTree);
    }
    return parsed;
}
