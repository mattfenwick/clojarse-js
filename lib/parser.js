...
var _char_octal = node('octal',
    ...
    char = node('char',
        ['open' , literal('\\')                                  ],
        ['value', alt(_char_octal, _char_unicode, _char_long, _char_simple, _char_catchall)]);
...
var _reserved = node('reserved',
    ...
    _ident = alt(seq2L(_reserved, not0(item)), _symbol),
    ...
    ident = bind(getState, function(pos) {
            return bind(_ident_pre, function(p) {
                ...
                switch (id.status) {
                    case 'success': return pure(id.value);
