...
var default_checks = [
    ...
    [type('list')               , [S.special_form, Mac.macro]         ],
    [type('symbol')             , [M.count_symbols]                   ],
//    [tag('struct')              , [M.indentation]                     ]
];

function traverse(checks, node, state, log, env) {
    var fs = [];
    ...
    switch (node._tag) {
        case 'token':
            break;
        case 'struct':
            node.elems.map(function(e) { traverse(checks, e, state, log, env); });
            break;
        default:
            throw new Error('unexpected AST node tag -- ' + JSON.stringify(node));
    }
    node.meta.map(function(m) { traverse(checks, m, state, log, env); });
}
