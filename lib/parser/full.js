...
//    console.log('working on ' + JSON.stringify([tree._name, tree._state]));
    ...
    } else if ( tree._name === 'number' ) {
        // well, this is a dreadful hack in numerous ways
        return {
            '_state': tree._state,
            '_name': tree._name,
            'value': tokens.parseToken({'_state': tree._state, '_name': tree._name, 
                                        'value': (tree.sign ? tree.sign : '') + tree.value})
        };
