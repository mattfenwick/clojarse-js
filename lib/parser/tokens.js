...
var clean = (function() {
    ...
    function cleanChar(node) {
        var type = node.value._name;
        if ( type === 'long' ) {
            return {
                '_name': 'char_long', // TODO uh, hack?
                '_state': node._state,
                'value': node.value.value
            };
        } else if ( type === 'unicode' ) {
            return {
                '_name': 'char_unicode', // TODO uh, hack?
                '_state': node._state,
                'value': node.value.first + node.value.rest.join('')
            };
        } else if ( type === 'octal' ) {
            return {
                '_name': 'char_octal', // TODO uh, hack?
                '_state': node._state,
                'value': node.value.first + node.value.rest.join('')
            };
        } else if ( type === 'simple' ) {
            return {
                '_name': 'char_simple', // TODO uh, hack?
                '_state': node._state,
                'value': node.value.value
            };
        } else {
    ...
    return function(node) {
        ...
        return node; // TODO raise error instead
    };
