...
function parse(type, input, pos) {
    if ( token_parsers.hasOwnProperty(type) ) {
        var p = token_parsers[type];
        return seq2L(cut('token', p),
                     cut('end', not0(item))).parse(input, pos).fmap(function(v) {return v.result;});
    }
