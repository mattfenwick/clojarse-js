...
var _base16 = node('base16',
    ...
    _baseN = node('baseN',
        ...
        ['r'    , oneOf('rR')                 ],
        ['body' , many1(alt(_0_9, _a_z, _A_Z))]), // TODO should commit to reaching end of input
...
var _char_octal = node('octal',
    ...
    char = node('char',
        ['value', alt(_char_octal, _char_unicode, _char_long, _char_simple)]);
...
function dirtyParse(token) {
    if ( token_parsers.hasOwnProperty(token._name) ) {
        ...
        return seq2L(cut('token', p),
                     cut('end', not0(item))).parse(token.value, token._start).fmap(function(v) {return v.result;});
    }
...
var clean = (function() {
    ...
    function cleanInteger(node) {
        ...
        if ( v._name === 'base8' ) {
            ...
            // why extract the base here?
            //   1. consistency with base10, base16, base8
            //   2. we know the string is a valid base10 number, because
            //      we parsed it -- so don't have to worry about errors
            out.base = parseInt(node.value.radix.join(''), 10);
            out.digits = node.value.body.join('');
            ...
            if ( c._name === 'char' ) {
                ...
                switch ( c.value._name ) {
                    case 'simple':
                        ...
                        break;
                    case 'octal':
                        out.value = c.value.digits.filter(isNotNull).join('');
                        break;
                    case 'unicode':
                        out.value = c.value.value.join('');
                        break;
                    default:
                        throw new Error('unrecognized escape in string -- ' + c.value._name);
                }
                return out;
            }
...
function parse_(type, string, position) {
    /*
     * convenience function: alternative interface to core `parse` function
     */
    return parse({'_name': type, '_start': position, 'value': string});
}
