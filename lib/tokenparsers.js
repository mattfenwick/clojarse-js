...
var _long_escapes = ["newline", "space", "tab", "backspace", "formfeed", "return"],
    ...
    _octal_check = function(ds) {
        if ( ds.length > 3 ) {
            throw new Error('what function do I need to use here????');
        } else if ( parseInt(ds, 8) > 255 ) {
            throw new Error('some kind of out of range error');
        }
        // otherwise, do I want to actually return the int?  no, right -- still just structure?
    }
    ...
    _char_octal = node('charoctal',
        ['o'     , literal('o')],
        ['digits', bind(many1(_0_7), _octal_check)]),
    ...
    char = node('char',
        ['value', any(_char_simple, _char_long, _char_octal, _char_unicode)]);
