...
/* these two probably belong in a different module */
RWS.warning = function(x) {
    function f(env, state) {
        return result(state, [{'warning': x}], null);
    }
    return new RWS(f);
}

// log an error -- not an error effect
RWS.error = function(x) {
    function f(env, state) {
        return result(state, [{'error': x}], null);
    }
    return new RWS(f);
}
