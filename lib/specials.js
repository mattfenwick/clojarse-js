...
function def(node, state, log, env) {
...
function if_(node, state, log, env) {
...
function monitor_exit(node, state, log, env) {
...
function do_(node, state, log, env) {
    // nothing to do
}

function dot(node, state, log, env) {
    arg_check('.', null, 2, error, 'too few arguments', node, log);
    // need to check whether 2nd arg is symbol or list
}

function set_bang(node, state, log, env) {
...
function recur(node, state, log, env) {
    // nothing to do
...
function _catch(node, state, log, env) {
...
function _finally(node, state, log, env) {
    arg_check('finally', null, 1, warning, 'missing expression(s)', node, log);
}

function try_(node, state, log, env) {
    var exprs = [],
        catches = [],
        final = null,
        i = 1,
...
function special_form(node, state, log, env) {
    ...
    return specials[first.name](node, state, log, env);
}
