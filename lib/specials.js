...
function def(node) {
    ...
    switch (elems.length) {
        case 1: error('def', 'too few arguments', node.pos); break;
        case 2: warning('def', 'missing initial value', node.pos); break;
        case 3: 
        case 4: break;
        default: error('def', 'too many arguments', node.pos); break;
...
function if_(node) {
    ...
    switch (elems.length) {
        case 1: 
        case 2: error('if', 'too few arguments', node.pos); break;
        case 3: warning('if', 'missing else-branch', node.pos); break;
        case 4: break;
        default: error('if', 'too many arguments', node.pos); break;
        case 0: throw new Error('should never have happened'); break;
    }
...
function quote(node) {
    ...
    switch (node.elems.length) {
        case 1: warning('quote', 'too few arguments', node.pos); break;
        case 2: break;
        default: warning('quote', 'too many arguments', node.pos); break;
        case 0: throw new Error('should never have happened'); break;
    }
}
...
function var_(node) {
    ...
    switch (node.elems.length) {
        case 1: error('var', 'too few arguments', node.pos); break;
        default: warning('var', 'too many arguments', node.pos);
...
function throw_(node) {
    ...
    switch (node.elems.length) {
        case 1: error('throw', 'too few arguments', node.pos); break;
        case 2: break;
        default: warning('throw', 'too many arguments', node.pos); break;
        case 0: throw new Error('should never have happened'); break;
    }
...
function new_(node) {
    ...
    switch (node.elems.length) {
        case 1: error('new', 'too few arguments', node.pos); break;
        default: break;
        case 0: throw new Error('should never have happened'); break;
    }
...
function monitor_enter(node) {
    ...
    switch (node.elems.length) {
        case 1: error('monitor-enter', 'too few arguments', node.pos); break;
        case 2: break;
        default: warning('monitor-enter', 'too many arguments', node.pos); break;
        case 0: throw new Error('should never have happened'); break;
    }
    node.elems.slice(1).map(dispatch); // TODO
}
...
function monitor_exit(node) {
    ...
    switch (node.elems.length) {
        case 1: error('monitor-exit', 'too few arguments', node.pos); break;
        case 2: break;
        default: warning('monitor-exit', 'too many arguments', node.pos); break;
        case 0: throw new Error('should never have happened'); break;
    }
    node.elems.slice(1).map(dispatch); // TODO
}
...
//    console.log('poop -- ' + JSON.stringify(node));
    switch (node._tag) {
        case 'token': return;
        case 'leaf': dispatch(node.form); return;
        case 'meta': dispatch(node.meta); dispatch(node.form); return;
        case 'struct': break;
        default: throw new Error('oops -- ' + node._tag);
    }
    switch (node.type) {
        case 'list': break;
        default: node.elems.map(dispatch); return;
    }
